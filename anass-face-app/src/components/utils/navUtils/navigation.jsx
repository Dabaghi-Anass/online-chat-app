    ...
    <div className="pt-4 fixed top-0 pb-2 bg-black bg-opacity-20 backdrop-blur-md w-full z-10">
      ...
      <div className="text-white text-4xl lg:hidden transition-all">
        ...
        <ul className='hidden h-100 lg:flex text-white text-3xl gap-16 items-center'>
          {links.map(link => (<li id={link.name} className="border-b-4 relative text-2xl isolate z-1 hover:rotate-6 border-transparent pb-2  hover:text-[#be068b80]" key={link.id}><NavLink to={link.path}>
            <span><ion-icon name={link.icon}></ion-icon>
            </span>
          </NavLink>
          </li>))}
        </ul>
